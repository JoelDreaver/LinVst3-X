...
*/
      ...
      chunk_ptr(0),
#endif
      m_inexcept(0), m_finishaudio(0), m_runok(0), m_syncok(0), m_386run(0),
      reaperid(0),
#ifdef EMBED
...
#ifdef EMBED
#ifdef EMBEDDRAG
void RemotePluginClient::eventloop(Display *display, Window pparent,
                                   Window parent, Window child, int width,
                                   int height, int parentok, int reaperid,
                                   RemotePluginClient *plugin)
#else
void RemotePluginClient::eventloop(Display *display, Window parent,
                                   Window child, int width, int height,
                                   int reaperid, RemotePluginClient *plugin)
#endif
{
  if (!display)
    return;
...
VstIntPtr RemotePluginClient::dispatchproc(AEffect *effect, VstInt32 opcode,
...
#else
      plugin->eventloop(plugin->display, plugin->parent, plugin->child,
  ...
  case effEditClose:
    if (plugin->editopen == 1) {
      ...
      plugin->xeclose = 1;
      plugin->sendXembedMessage(plugin->display, plugin->child,
                                XEMBED_EMBEDDED_NOTIFY, 0, plugin->parent, 0);
      ...
      plugin->syncevents(plugin->display);
 ...
 */
#ifdef EMBED
      if (execlp("/usr/bin/lin-vst3-server-x.exe",
                 "/usr/bin/lin-vst3-server-x.exe", NULL, NULL)) {
        m_runok = 1;
        cleanup();
        return;
      }
#else
      if (execlp("/usr/bin/lin-vst3-server-xst.exe",
                 "/usr/bin/lin-vst3-server-xst.exe", NULL, NULL)) {
        m_runok = 1;
        cleanup();
        return;
      }
