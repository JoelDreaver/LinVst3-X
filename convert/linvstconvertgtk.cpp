...
void buttoncallback(GtkFileChooser *button, gpointer data)
{
 ...
 if(name.find("linvst3x.so") == std::string::npos)
 {
...
int main (int argc, char *argv[])
{
  ...
  gtk_window_set_title (GTK_WINDOW (window), "LinVst3-X");
  gtk_container_set_border_width (GTK_CONTAINER (window), 8);

  spacertext = gtk_label_new ("Choose linvst3x.so");
  ...
  fileselect = gtk_file_chooser_button_new ("Choose linvst3x.so", GTK_FILE_CHOOSER_ACTION_OPEN);
  ...
  gtk_file_filter_add_pattern (extfilter, "linvst3x.so");
